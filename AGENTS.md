...
2. **Regenerate and test** with `node tools/check.js`. It assembles the
   self-contained homepage, validates the feature registry and scaffolder,
   then runs `tools/calcpage.js --all`, `tools/sitemap.js`,
   `tools/prerender.js`, and `tests/run.js` in dependency order. Never run them out of
...
re-parses `src/site/scripts/data.js` to confirm its own edit was sound and
...
A static site. Self-contained HTML files at the repository root are served
directly with no deployment-time build step. Modular authoring sources are
assembled at commit time:

| File | What it is |
|---|---|
| `index.html` | Generated, committed homepage artifact — sidebar shell, live command overview, capability plates, About/Record section, 14 domains · 34 calculators · 103 metrics, and the `.xlsx` writer |
...
| `src/site/` | Canonical homepage authoring source: shell template, shared CSS layers, and ordered JavaScript modules. `tools/assemble.js` inlines them into `index.html` |
| `src/features/manifest.json` | Feature registry: stable id, category, kind, and instrument family for every shipped or draft feature |
...
generator reads those plus the assembled calculator definitions and writes one
self-contained page per manifest entry.

After editing calculator data in `src/site/scripts/data.js`, shared source under
`src/site/`, the feature manifest, **or** any file under `content/`, run the
complete pipeline and commit its output:

```bash
node tools/check.js
```

The assembler runs before the existing generators so every downstream tool
reads the same committed artifact. Run individual scripts only when debugging:
...
- **The cut is anchored on indentation.** It finds card literals by their
  ...
  the definitions in `src/site/scripts/data.js` and the anchors stop matching. That is a
...
- **No serve-time build step.** No bundler, transpiler, framework, server-side
  rendering, or deployment compilation. `tools/assemble.js` is a dependency-free
  commit-time assembler; the committed root HTML remains the shipped artifact.
...
- **Self-contained output.** Every published page carries its CSS in `<style>`
  and application JavaScript in `<script>` — except the external Google tag.
  Authoring source may be shared under `src/`; the assembler and page generator
  must inline it so visitors never depend on those source files.
...
- **Defined browser baseline.** Shipped code targets the current two stable
  releases of Chrome, Edge, Firefox, and Safari without transpilation. Existing
  shared modules retain their `var`/IIFE/CommonJS-compatible shape until a whole
  module is intentionally modernised; isolated new modules may use supported
  modern syntax. The Google tag is Google's snippet and is the exception.

## Design language

The global shell is a project-controls Command Deck styled as a bench of
calibrated instruments. Light mode is the
...
Seven scoped instrument families give domains distinct material character
without changing navigation or status meaning: `control-room`, `field-notebook`,
`survey-map`, `planning-wall`, `drafting-table`, `signal-station`, and
`laboratory`. A feature selects one in `src/features/manifest.json`. Family
tokens may affect category headers, rules, chart ornament, and inactive context;
they may not redefine `--good`, `--warn`, `--bad`, typography legibility, focus,
or interaction behavior. The homepage Review Lens is the single signature
interaction and must retain written selection plus `aria-pressed` state.

Use the canonical custom properties in `src/site/styles/` when adding a page;
the generated HTML is output, not the editing surface.
