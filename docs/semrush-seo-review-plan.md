...
**Updated:** 2 September 2026 — phase-one code shipped on `main` (`05c24c5`). The Evidence table below is still the 1 September snapshot; **Progress** is current.
...
**Status:** Phase-one **code is live.** Owner measurement (Search Console, Position Tracking) and the HSTS-from-outside check are not. Phase two waits on indexing and impressions.
...
**The highest-impact first phase is five items.** Items 2, 3 and 5 (code) are
shipped; item 1 (Search Console) and item 4 (measurement baseline) are still
the owner. Everything else waits for GSC data.
...
Shipped on `main` (Hostinger deploys from this branch). Day 0 of the 30/60/90
targets still starts when Search Console is verified; the pilot page being
live does not start that clock by itself.

| Item | State |
|---|---|
| **1. Google Search Console** | **Not done.** Owner: Domain-property DNS TXT at Hostinger, submit `sitemap.xml`, URL Inspection on `/` and `/earned-value-analysis.html`, request indexing. The GA4 tag does not replace this. |
| **2. Earned-value pilot** | **Done.** `earned-value-analysis.html` generated by `tools/calcpage.js`; prose in `content/earned-value-analysis.html` (~1,700 words); six pre-merge fixes applied (breadcrumb `item`, `stateUrl()` from `location`, live footer count, byline → `/#about-h`, `updated` 2026-09-02, cluster H2s). Desk link in both `buildCard` (`PM_DATA.page`) and the prerender mirror. `tests/pages.js` gates freshness, ≥1,000 words, breadcrumbs, footer count, static homepage `href`, sitemap loc, and the Google tag. |
| **3. Homepage as hub** | **Done.** Title, meta, Open Graph, JSON-LD, hero H1 and lede lead with the desk (`34 calculators. Across 14 domains.`). `og.png` not re-rendered — figures still 14 · 34 · 103. |
| **4. Measurement baseline** | **Not done.** Owner: Semrush Position Tracking (~15 terms, US + SA); weekly / fortnightly / monthly funnel. |
| **5. HSTS + font cache** | **Committed.** `.htaccess` has a guarded `<IfModule mod_headers.c>` with `Strict-Transport-Security: max-age=300` (no `includeSubDomains` / preload) and `Cache-Control` on `.woff2`. **Unverified from outside** — CDN may strip origin headers. Ramp the `max-age` only after `curl -I` shows the 300s header. |
| **404 font paths** | **Done.** Root-relative `/fonts/...` (was listed under phase two). |
| **`AGENTS.md`** | **Done.** Per-calculator generator, regenerate-after-edit rule, prose gate, module guards, GA4 as the named third-party exception. |
| **Google Analytics 4** | **Done, after the plan.** Owner chose to drop the “nothing is sent anywhere” promise. Tag `G-J4XTN125MF` on every shipped page; `page_location` omits the hash so calculator inputs in a copied deep-link are not sent. Footers and meta state that Analytics records the visit. Not a GSC substitute. |
| **URL form** | **Kept `.html`.** No extensionless rewrite. |
| **Wave one size** | **Pilot only** — no PERT page in this wave. |

**Still not in the code, and still phase two:** verdict sentences as data; PERT / three-point page; PMP formula-sheet page; whole-domain risk page; mirror fragment ids (`#prerender-*` vs `#calc-*`); outreach.
...
*Snapshot of 1 September 2026. For what shipped after that date, see
**Progress — 2 September 2026**.*
...
**Fixes the draft needed before merge** — all six applied on `main` (2 September 2026):
...
| **404 page** *(not in the plan)* | Cosmetic | Returns a real 404. Font paths were relative on 1 September; they are now root-relative (`/fonts/...`). No ranking effect |
...
| # | Item | State (2 Sep 2026) | Depends on | Success metric | Owner decision? |
|---|---|---|---|---|---|
| 1 | **Verify Google Search Console** for `yazeed.blog` as a Domain property (DNS TXT at Hostinger); submit `sitemap.xml`; run URL Inspection on the homepage and the pilot page and record indexed / Google-selected canonical / rendered headings visible | **Not done** | — | Property verified; sitemap "Success"; homepage "URL is on Google" with the apex as canonical | Yes — DNS or hosting-panel access |
| 2 | **Ship the earned-value pilot page**: six pre-merge fixes; homepage link in **both** the card template (`PM_DATA`) and the prerender mirror; `tests/pages.js` assertion and ≥1,000-word gate; sitemap entry; CLS check; merge; request indexing in GSC | **Code done.** Request indexing still the owner (needs item 1) | 1 for the GSC half | Page returns 200 live (yes); "URL is on Google" within 14 days; impressions for any earned-value query within 60 days | Decided: generator sanctioned; `.html` URLs kept; prose shipped |
| 3 | **Re-title the homepage as the hub** so it stops competing with item 2: title, meta description, `og:title`, `og:description`, hero H1 and lede lead with the desk, not with earned value; keep a count phrase in every field `tests/counts.js` checks; re-render `og.png` only if its figures change | **Done.** `og.png` unchanged | 2 | Homepage and pilot page show impressions for different query sets in GSC; no double-listing for one query | Decided: desk-first |
| 4 | **Baseline the measurement**: Position Tracking for ~15 intent-matched terms in US and SA, split from mismatched controls; the weekly / fortnightly / monthly funnel above; a Semrush re-baseline four weeks after the prerender and canonical fix went live | **Not done** | 1 | Baseline recorded with sources; first review completed | No |
| 5 | **Security hygiene in one guarded block**: `<IfModule mod_headers.c>` in `.htaccess` carrying `Header always set Strict-Transport-Security "max-age=300"` and `Cache-Control` for `/fonts/`; verify both headers from outside after deploy; ramp `max-age` on the documented schedule; if the CDN strips them, set HSTS in Hostinger's panel | **Committed.** Outside `curl -I` and any CDN fallback **not done** | — | `curl -I` from outside shows both headers; Semrush HSTS notice clears on the next audit | No — confirm no other subdomains exist before `includeSubDomains` |
...
- **Fix the mirror's fragment ids** (`#prerender-*` vs `#cat-*` / `#calc-*`)
  so a deep link extracted from the static HTML lands on the live card. The
  404 page's font paths were already made root-relative in phase one.
...
Resolved 2 September 2026 (implied by shipping phase one on `main`):

1. **`tools/calcpage.js` is the sanctioned way to add pages** — commit-time
   generation in the `tools/prerender.js` pattern. `AGENTS.md` describes the
   generator, `content/`, `tests/pages.js`, the regenerate-after-editing rule
   and the prose gate.
2. **Homepage is desk-first**, not earned-value-first.
3. **URL form:** `.html` kept. No extensionless rewrite.
7. **Wave one** is the earned-value pilot alone. PERT waits for phase two.
10. **Google Analytics 4** (`G-J4XTN125MF`) — added after the plan, which
    forbade third-party requests. Footers no longer claim that nothing is
    sent anywhere. Still not a substitute for Search Console.

Still open:

4. **Google Search Console:** who holds DNS or hosting-panel access to verify
   the Domain property? *(Blocks the measurement clock.)*
...
- Whether Hostinger's CDN passes `Header` directives from `.htaccess` to the
  edge is **still unverified** after the 2 September deploy; check with
  `curl -I` from outside. If `hcdn` strips them, set HSTS in Hostinger's panel.
...
**2 September 2026.** Phase-one items 2, 3 and 5 (code) and the 404 font-path
fix shipped on `main`. Google Analytics was added as a later owner decision.
Items 1 and 4, the outside header check, and all of phase two remain.
